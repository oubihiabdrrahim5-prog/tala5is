...
const Header: React.FC<HeaderProps> = ({ user, onAuthClick, onLogout, onGoHome, onViewChange }) => {
    ...
    <header className="glass-card sticky top-0 z-50 w-full shadow-sm border-b border-white/50">
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-10">
          <button onClick={onGoHome} className="text-sm font-bold text-slate-500 hover:text-blue-600">الرئيسية</button>
          ...
          {user?.role === 'admin' && (
            <button onClick={() => onViewChange('dashboard')} className="text-sm font-black text-blue-600 flex items-center gap-2">
